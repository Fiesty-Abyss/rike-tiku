...
未合并分支 `feat/demo-question-bank-expansion` 将独立 `rike_tiku_demo` 扩充为 Demo90：物理、化学、生物各 30 道项目原创自编演示题，每科三题型、三档难度和三个演示知识点各 10 道。Demo90 仅用于稳定演示筛选、随机练习、判分和错题链路，不等于 MVP30 正式真实题库；MVP30 仍未正式入库，网络候选题也未因此改为 `PUBLISHED`。本轮不新增 Flyway 迁移，不写正式 `rike_tiku`。
