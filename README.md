...
已完成前端认证基础：三角色登录入口、Pinia认证状态、Bearer Token注入、会话恢复、首次改密、路由守卫和最小工作台占位。尚未完成：学生导入、用户管理、题库正式业务API、练习判分、错题、AI Provider和正式角色工作台。题库30题候选数据尚未正式发布。
...
本轮前端认证基础完成后，下一阶段需由单独任务决定；不同时开发学生导入、题库、练习或AI。
