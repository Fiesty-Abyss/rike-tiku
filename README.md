...
- 管理员学生Excel确认入库后端：整批事务创建账号、STUDENT角色、学生档案和主班级关系（当前PR尚未合并）。

已完成前端认证基础：三角色登录入口、Pinia认证状态、Bearer Token注入、会话恢复、首次改密、路由守卫和最小工作台占位。尚未完成：班级管理前端、学生导入、用户管理、教师管理、任课关系管理、题库正式业务API、练习判分、错题、AI Provider和正式角色工作台。题库30题候选数据尚未正式发布。
...
学生Excel模板与预检查后端已合并并完成main回归；下一阶段应单独实现确认入库后端，不同时开发前端页面、教师导入、题库、练习或AI。
