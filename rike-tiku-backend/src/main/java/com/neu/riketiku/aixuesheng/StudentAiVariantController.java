...
public class StudentAiVariantController {
    ...
    @PostMapping public StudentAiVariantDtos.Variant generate(@AuthenticationPrincipal RenZhengYongHu user,@Valid @RequestBody StudentAiVariantDtos.Generate request){return service.generate(user.id(),request.answerFactId(),request.targetDifficulty());}
