...
public class StudentAiVariantService {
    ...
    private RenZhengYeWuYiChang translateGenerationFailure(RenZhengYeWuYiChang exception){
        ...
        if(code.startsWith("AI_CANDIDATE_")){String message=code.contains("EMPTY")?"AI 返回内容为空":code.contains("OPTIONS")?"AI 返回的选项不完整":code.contains("ANSWER_OPTION")?"AI 返回的答案与选项不一致":code.contains("SIMILARITY")?"AI 生成内容与母题过于相似":"AI 返回的题目字段未通过校验";return error(code,message,HttpStatus.SERVICE_UNAVAILABLE);}
    ...
    private record Fact(long studentId,long questionId,String type,int difficulty){} private record Row(long id,long factId,long motherId,long questionId,String status,String variationMode,String type,String stem,int difficulty,String optionsJson,String correctAnswerJson,String studentAnswerJson,Boolean correct,String analysis,String reviewStatus){}
}
