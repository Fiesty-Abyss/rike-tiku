...
public class TeacherPrivateQuestionService{
 ...
 @Transactional(readOnly=true) public QuestionDtos.Detail detail(long userId,long id){one(userId,id);return questions.detail(id);}
 ...
 @Transactional public Item update(long userId,long id,QuestionDtos.Save save){Item item=one(userId,id);if(!"DRAFT".equals(item.status()))throw fail("PRIVATE_QUESTION_STATUS_INVALID","只有草稿可以编辑",HttpStatus.CONFLICT);authorize(userId,item.teachingAssignmentId(),save.subjectId());questions.update(id,save);return one(userId,id);}
 ...
 @Transactional public Item submitAdmin(long userId,long id){Item item=one(userId,id);if(!"DRAFT".equals(item.status()))throw fail("PRIVATE_QUESTION_STATUS_INVALID","只有草稿可以提交管理员",HttpStatus.CONFLICT);jdbc.update("UPDATE ti_mu SET ke_jian_fan_wei='GLOBAL',ren_ke_guan_xi_id=NULL,zhuang_tai='PENDING' WHERE id=?",id);jdbc.update("UPDATE ti_mu_jie_xi SET zhuang_tai='PENDING' WHERE ti_mu_id=?",id);return one(userId,id);}
 @Transactional public Item disable(long userId,long id){Item item=one(userId,id);jdbc.update("UPDATE ti_mu SET zhuang_tai='DISABLED' WHERE id=?",id);jdbc.update("UPDATE ti_mu_jie_xi SET zhuang_tai='DISABLED' WHERE ti_mu_id=?",id);return one(userId,id);}
 @Transactional public void delete(long userId,long id){Item item=one(userId,id);if(!"DRAFT".equals(item.status()))throw fail("PRIVATE_QUESTION_STATUS_INVALID","只有未引用草稿可以删除",HttpStatus.CONFLICT);Long used=jdbc.queryForObject("SELECT COUNT(*) FROM shi_juan_ti_mu WHERE ti_mu_id=?",Long.class,id);if(used!=null&&used>0)throw fail("PRIVATE_QUESTION_REFERENCED","已被试卷引用的题目只能停用",HttpStatus.CONFLICT);jdbc.update("UPDATE ti_mu SET yi_shan_chu=1 WHERE id=?",id);}
