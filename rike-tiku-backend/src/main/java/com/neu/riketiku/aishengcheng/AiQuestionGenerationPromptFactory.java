...
class AiQuestionGenerationPromptFactory {
    ...
    private static final String SYSTEM="""
            ...
            changedDimensions 只能使用 SCENARIO、CONDITION、DATA、REPRESENTATION、REASONING_PATH、DISTRACTOR、KNOWLEDGE_COMBINATION，且至少两个。情境迁移必须包含 SCENARIO+DATA 或 SCENARIO+CONDITION；条件重组必须包含 CONDITION+REASONING_PATH 或 CONDITION+DATA；表达形式转换必须包含 REPRESENTATION+REASONING_PATH；多步骤扩展必须包含 REASONING_PATH+CONDITION；干扰项重构必须包含 DISTRACTOR+REASONING_PATH；综合变式至少三个维度。
    ...
    AiModelRequest noveltyRepair(String invalidJson,String reason,int expectedCount,String expectedType,int expectedDifficulty,String expectedMode,int maxTokens){
        Map<String,Object> data=new LinkedHashMap<>();
        data.put("candidateJson",safe(invalidJson));
        data.put("rejectionReason",safe(reason));
        data.put("expectedCount",expectedCount);
        data.put("expectedType",expectedType);
        data.put("expectedDifficulty",expectedDifficulty);
        data.put("expectedVariationMode",expectedMode);
        data.put("variationRequirement",variationRequirement(expectedMode));
        return model("AI_QUESTION_GENERATION_NOVELTY_REPAIR","novelty-repair","上一候选与母题过于接近。不要只替换数字、单位、人物或选项顺序；请改变情境、条件、数据结构或推理路径，保持题型、知识点和答案可验证，并返回完整 schemaVersion 2 JSON。UNTRUSTED_NOVELTY_REPAIR_DATA="+mapper.writeValueAsString(data),maxTokens);
    }
    private String variationRequirement(String mode){return switch(mode){case "SCENARIO_TRANSFER"->"SCENARIO+DATA 或 SCENARIO+CONDITION";case "CONDITION_RECOMBINATION"->"CONDITION+REASONING_PATH 或 CONDITION+DATA";case "REPRESENTATION_SWITCH"->"REPRESENTATION+REASONING_PATH";case "MULTI_STEP_EXTENSION"->"REASONING_PATH+CONDITION";case "DISTRACTOR_REDESIGN"->"DISTRACTOR+REASONING_PATH";case "COMBINED"->"至少三个不同变化维度";default->"至少两个不同变化维度";};}
