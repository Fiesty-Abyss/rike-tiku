...
describe('路由守卫', () => {
  ...
  it('管理员可访问班级、学生导入和教师管理页面', async () => { authenticated(['ADMIN']); await router.push('/admin/classes'); expect(router.currentRoute.value.path).toBe('/admin/classes'); await router.push('/admin/students/import'); expect(router.currentRoute.value.path).toBe('/admin/students/import'); await router.push('/admin/teachers'); expect(router.currentRoute.value.path).toBe('/admin/teachers') })
