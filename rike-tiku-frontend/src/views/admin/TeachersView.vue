...
const assignmentRef = ref<FormInstance>();
...
const form = reactive({
  ...
  profileStatus: "ACTIVE" as TeacherProfileStatus,
});
...
const profileStatuses: Array<{ label: string; value: TeacherProfileStatus }> = [
  { label: "有效", value: "ACTIVE" },
  { label: "停用", value: "DISABLED" },
];
const assignmentStatuses: Array<{
  label: string;
  value: TeachingAssignmentStatus;
}> = [
  { label: "任课中", value: "ACTIVE" },
...
const teacherRules: FormRules = {
  ...
  initialPassword: [
    { min: 8, max: 64, message: "初始密码需为8至64位", trigger: "blur" },
  ],
};
...
function message(error: unknown, fallback: string) {
  ...
  const map: Record<string, string> = {
    TEACHER_NUMBER_EXISTS: "工号已存在。",
    USERNAME_EXISTS: "用户名已存在。",
    TEACHER_ROLE_UNAVAILABLE: "TEACHER角色不可用，请联系系统管理员。",
    TEACHING_ASSIGNMENT_EXISTS:
      "该教师、班级、科目关系已存在，历史关系不可重复创建。",
    TEACHER_UNAVAILABLE: "教师账号或档案不是有效状态。",
    CLASS_UNAVAILABLE: "班级不存在或不是在读状态。",
...
async function loadTeachers() {
  ...
  try {
    ...
    ElMessage.error(message(error, "教师列表加载失败。"));
  } finally {
    loading.value = false;
  }
}
...
async function openEdit(item: TeacherItem) {
  try {
    const data = await fetchTeacher(item.id);
    editing.value = true;
    Object.assign(form, data.teacher, { initialPassword: "" });
    teacherDialog.value = true;
  } catch (error) {
...
async function saveTeacher() {
  ...
  try {
    ...
    ElMessage.error(message(error, "教师保存失败。"));
  } finally {
    saving.value = false;
  }
}
async function showDetail(item: TeacherItem) {
  try {
...
async function openAssignment() {
  ...
  try {
    const [classPage, subjectData] = await Promise.all([
      fetchClasses({ page: 1, size: 100, status: "ACTIVE" }),
      fetchSubjects(),
    ]);
    ...
    ElMessage.error(message(error, "班级或科目数据加载失败。"));
  }
}
async function saveAssignment() {
  const valid = await assignmentRef.value?.validate().catch(() => false);
  if (
    !valid ||
    !detail.value ||
  ...
  saving.value = true;
  try {
    await createTeachingAssignment(detail.value.teacher.id, {
      classId: assignment.classId,
      subjectId: assignment.subjectId,
      primary: assignment.primary,
      startTime: new Date(assignment.startTime).toISOString(),
    });
    detail.value = await fetchTeacher(detail.value.teacher.id);
    assignmentDialog.value = false;
    ElMessage.success("任课关系已创建。");
  } catch (error) {
    ElMessage.error(message(error, "任课关系创建失败。"));
  } finally {
    saving.value = false;
  }
}
async function updateAssignmentStatus(
  ...
  try {
    await changeTeachingAssignmentStatus(id, status);
    if (detail.value)
      detail.value = await fetchTeacher(detail.value.teacher.id);
    ElMessage.success("任课关系状态已更新。");
  } catch (error) {
    ElMessage.error(message(error, "任课关系状态更新失败。"));
  }
}
  ...
  <section class="admin-page">
    ...
    <el-form
      ...
      ><el-form-item label="工号"
        ><el-input v-model="filters.employeeNumber" clearable /></el-form-item
      ><el-form-item label="姓名"
        ><el-input v-model="filters.name" clearable /></el-form-item
      ><el-form-item label="用户名"
        ><el-input v-model="filters.username" clearable /></el-form-item
      ><el-form-item label="账号"
        ><el-select v-model="filters.accountStatus" clearable
          ><el-option
            v-for="item in accountStatuses"
            :key="item.value"
            :label="item.label"
            :value="item.value" /></el-select></el-form-item
      ><el-form-item label="档案"
        ><el-select v-model="filters.profileStatus" clearable
          ><el-option
            v-for="item in profileStatuses"
            :key="item.value"
            :label="item.label"
            :value="item.value" /></el-select></el-form-item
      ><el-form-item
        ...
        ><el-button :loading="loading" @click="loadTeachers"
          >刷新</el-button
        ></el-form-item
      ></el-form
    >
      ...
      ><el-table-column label="账号状态" min-width="110"
        ><template #default="{ row }"
          ><el-tag
            ...
            >{{ label(accountStatuses, row.accountStatus) }}</el-tag
          ></template
        ></el-table-column
      ><el-table-column label="档案状态" min-width="110"
        ><template #default="{ row }"
          ><el-tag
            :type="row.profileStatus === 'ACTIVE' ? 'success' : 'info'"
      ...
      >
        ...
        ><el-form-item label="用户名" prop="username"
          ><el-input
            ...
            :disabled="editing" /></el-form-item
        ><el-form-item label="展示职务"
          ><el-input v-model="form.displayPosition" /></el-form-item
        ><el-form-item v-if="!editing" label="初始密码" prop="initialPassword"
          ><el-input
            v-model="form.initialPassword"
            type="password"
            show-password
            placeholder="不填则生成随机密码" /></el-form-item
        ><el-form-item label="账号状态"
          ><el-select v-model="form.accountStatus"
            ><el-option
              ...
              :label="item.label"
              :value="item.value" /></el-select></el-form-item
        ><el-form-item v-if="editing" label="档案状态"
          ><el-select v-model="form.profileStatus"
            ><el-option
              v-for="item in profileStatuses"
              :key="item.value"
              :label="item.label"
              :value="item.value" /></el-select></el-form-item></el-form
      ><template #footer
        ><el-button @click="teacherDialog = false">取消</el-button
        ><el-button type="primary" :loading="saving" @click="saveTeacher"
          >保存</el-button
        ></template
      ></el-dialog
    >
      ...
      ><template v-if="detail"
        ><div class="teacher-detail">
          <el-descriptions :column="2" border
            ...
            ><el-descriptions-item label="展示职务">{{
              detail.teacher.displayPosition || "—"
            }}</el-descriptions-item
            ><el-descriptions-item label="账号 / 档案"
              >{{ label(accountStatuses, detail.teacher.accountStatus) }} /
              {{
                label(profileStatuses, detail.teacher.profileStatus)
              }}</el-descriptions-item
            ></el-descriptions
          >
          ...
          <el-table
            :data="detail.teachingAssignments"
            class="data-table"
            empty-text="暂无任课关系。"
            ><el-table-column
              prop="classCode"
            ...
            /><el-table-column
              ...
              label="科目"
              min-width="100"
            /><el-table-column label="主任课" min-width="90"
              ><template #default="{ row }">{{
                row.primary ? "是" : "否"
            ...
            ><el-table-column label="状态" min-width="110"
              ><template #default="{ row }"
                ><el-tag :type="row.status === 'ACTIVE' ? 'success' : 'info'">{{
                  label(assignmentStatuses, row.status)
                }}</el-tag></template
              ></el-table-column
            ><el-table-column
              prop="startTime"
            ...
            /><el-table-column label="操作" min-width="130"
              ><template #default="{ row }"
                ><el-dropdown
                  @command="
                    (status: TeachingAssignmentStatus) =>
                      updateAssignmentStatus(row.id, status)
                  "
                  ><el-button link type="primary">变更状态</el-button
                  ><template #dropdown
                    ><el-dropdown-menu
                      ><el-dropdown-item
                        v-for="item in assignmentStatuses"
                        :key="item.value"
                        :command="item.value"
    ...
    <el-dialog
      ...
      /><el-form
        ...
        ><el-form-item label="科目" prop="subjectId"
          ><el-select v-model="assignment.subjectId" style="width: 100%"
            ><el-option
              v-for="item in subjects"
              :key="item.id"
              :label="item.subjectName"
              :value="item.id" /></el-select></el-form-item
        ><el-form-item label="主任课"
          ><el-switch v-model="assignment.primary" /></el-form-item
