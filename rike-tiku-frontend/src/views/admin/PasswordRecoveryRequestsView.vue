...
import { onMounted,ref } from 'vue';import { ElMessage,ElMessageBox } from 'element-plus';import { fetchPasswordRecoveries,rejectPasswordRecovery,resolvePasswordRecovery,type PasswordRecoveryItem } from '../../api/admin/passwordRecovery'
...
<template><section v-loading="loading"><el-table :data="records"><el-table-column prop="username" label="账号"/><el-table-column prop="name" label="姓名"/><el-table-column prop="role" label="角色"/><el-table-column prop="requestedAt" label="申请时间"/><el-table-column prop="status" label="状态"/><el-table-column label="操作"><template #default="{row}"><template v-if="row.status==='PENDING'"><el-button type="primary" @click="resolve(row)">恢复默认密码</el-button><el-button @click="reject(row)">驳回</el-button></template></template></el-table-column></el-table></section></template>
