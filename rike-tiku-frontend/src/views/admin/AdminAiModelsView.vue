...
const formIndex=(usage:string)=>({TEXT:0,VISION:1,SEARCH:2} as Record<string,number>)[usage]
const usageTitle=(usage:string)=>({TEXT:'DeepSeek 文本推理',VISION:'GLM 视觉理解',SEARCH:'智谱联网搜索'} as Record<string,string>)[usage]
const usageKicker=(usage:string)=>({TEXT:'TEXT BRAIN',VISION:'VISION EYES',SEARCH:'SEARCH SOURCES'} as Record<string,string>)[usage]
async function load(){loading.value=true;try{const page=await fetchAiModels();for(const item of page.records){const index=formIndex(item.usage);if(index!==undefined)apply(index,item)}}catch(e){ElMessage.error(errorText(e,'AI 模型配置加载失败。'))}finally{loading.value=false}}
async function save(form:Form){saving.value=form.usage;try{const body:SaveAiModelConfig={provider:form.provider,model:form.model,baseUrl:form.baseUrl,apiKey:form.apiKey?.trim()||undefined,usage:form.usage,enabled:form.enabled,defaultConfig:form.defaultConfig,timeoutMillis:form.timeoutMillis,maxTokens:form.maxTokens,retryCount:form.retryCount};const item=form.id?await updateAiModel(form.id,body):await createAiModel(body);apply(formIndex(form.usage),item);ElMessage.success('模型配置已保存并立即生效。')}catch(e){ElMessage.error(errorText(e,'模型配置保存失败。'))}finally{saving.value=undefined}}
async function clearKey(form:Form){if(!form.id)return;await ElMessageBox.confirm('清除后该模型将不可调用。','确认清除 API Key',{type:'warning'});try{apply(formIndex(form.usage),await clearAiModelKey(form.id));ElMessage.success('API Key 已清除。')}catch(e){ElMessage.error(errorText(e,'API Key 清除失败。'))}}
async function test(form:Form){if(!form.id){ElMessage.warning('请先保存配置。');return}testing.value=form.usage;try{const result=await testAiModel(form.id);await load();result.success?ElMessage.success(`${result.model} 连接成功，耗时 ${result.latencyMillis} ms。`):ElMessage.warning(`${result.httpStatus?`HTTP ${result.httpStatus} · `:''}${result.safeErrorCode||'UNKNOWN'} · ${result.safeError||'连接测试失败'} · ${result.latencyMillis} ms`)}catch(e){ElMessage.error(errorText(e,'连接测试失败。'))}finally{testing.value=undefined}}
  ...
  <section class="admin-page ai-model-page" v-loading="loading">
    ...
    <div class="ai-model-grid">
      <article v-for="form in forms" :key="form.usage" class="ai-model-card">
        <header><div><span>{{ usageKicker(form.usage) }}</span><h2>{{ usageTitle(form.usage) }}</h2></div><el-switch v-model="form.enabled" inline-prompt active-text="启用" inactive-text="停用" /></header>
        <el-form label-position="top" @submit.prevent="save(form)">
          <div class="ai-model-fields"><el-form-item label="Base URL"><el-input v-model="form.baseUrl" /></el-form-item><el-form-item label="模型 / 引擎"><el-select v-model="form.model"><template v-if="form.usage==='TEXT'"><el-option label="deepseek-v4-flash" value="deepseek-v4-flash" /><el-option label="deepseek-v4-pro" value="deepseek-v4-pro" /></template><el-option v-else-if="form.usage==='VISION'" label="glm-4.6v-flash" value="glm-4.6v-flash" /><template v-else><el-option label="search_std" value="search_std" /><el-option label="search_pro" value="search_pro" /><el-option label="search_pro_sogou" value="search_pro_sogou" /><el-option label="search_pro_quark" value="search_pro_quark" /></template></el-select></el-form-item></div>
          ...
          <footer><el-button type="primary" native-type="submit" :loading="saving===form.usage">保存配置</el-button><el-button :loading="testing===form.usage" @click="test(form)">测试连接</el-button><el-button v-if="form.apiKeyConfigured" type="danger" plain @click="clearKey(form)">清除 Key</el-button></footer>
        </el-form>
      </article>
    </div>
  </section>
