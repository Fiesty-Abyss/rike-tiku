...
const usageModeLabel=(value:string)=>({ONLINE_PRACTICE:'在线练习',TOPIC_LEARNING:'专题学习'} as Record<string,string>)[value]||value
const subjectLabel=(value:string)=>({PHYSICS:'物理',CHEMISTRY:'化学',BIOLOGY:'生物'} as Record<string,string>)[value]||value
...
async function confirmImport() {
  ...
  try {
    await ElMessageBox.confirm('确认导入当前文件中的全部有效题目？导入后可前往题库审核列表继续处理。', '确认导入题库', { type:'warning', confirmButtonText:'确认入库', cancelButtonText:'返回检查' })
  } catch { return }
...
<template>
  <section class="admin-page question-import-page">
    <div class="page-heading"><div><h1>题库 Excel 导入</h1><p>上传后先逐行预检查，确认导入的题目进入待审核列表。</p></div><el-button @click="router.push('/admin/questions')">返回题库审核</el-button></div>
    ...
    <section v-if="preview" class="import-stage"><div class="section-title-row"><div><h2>2. 逐行预检查</h2><p>学科：{{ preview.subjectCode ? subjectLabel(preview.subjectCode) : '未确认' }}</p></div><el-button :loading="previewing" @click="runPreview">重新预检查</el-button></div>
    ...
    <section v-if="result" class="import-stage"><h2>3. 导入完成</h2><el-alert :title="`已导入 ${result.importedCount}/${result.totalCount} 道题目，全部为待审核状态。`" type="success" :closable="false" show-icon /><div class="stage-actions"><el-button type="primary" @click="router.push('/admin/questions')">前往题库审核列表</el-button><el-button @click="clear">继续导入下一文件</el-button></div></section>
  </section>
</template>
