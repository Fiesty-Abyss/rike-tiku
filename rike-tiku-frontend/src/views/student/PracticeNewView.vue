...
async function loadPoints() { if (!form.subjectId) return; try { points.value=(await fetchPracticeOptions(form.subjectId)).knowledgePoints; const availableIds=new Set(points.value.map(point=>point.id)); form.knowledgePointIds=form.knowledgePointIds.filter(id=>availableIds.has(id)) } catch (error) { ElMessage.error(message(error)) } }
