...
onMounted(async()=>{ try { const data=await fetchPracticeOptions(); subjects.value=data.subjects; form.subjectId=subjects.value[0]?.id || 0; await loadPoints() } catch(error){ ElMessage.error(message(error)) } })
