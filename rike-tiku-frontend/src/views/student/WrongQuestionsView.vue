...
import { archiveWrongQuestion, fetchPracticeOptions, fetchWrongQuestion, fetchWrongQuestions, retryWrongQuestion, type KnowledgePoint, type Subject, type WrongQuestion, type WrongQuestionDetail, type WrongQuestionFilterStatus } from '../../api/student/practice'
...
async function retry(item:WrongQuestion){try{const session=await retryWrongQuestion(item.questionId);await router.push({path:`/student/practice/${session.id}`,query:{retryQuestionId:item.questionId}})}catch(error){ElMessage.error((error as ApiError).message||'暂时无法创建复习会话。')}}
async function archive(item:WrongQuestion){try{await ElMessageBox.confirm('仅从活跃错题本移出；历史答题、错误次数和 AI 分析都会保留。','移出当前错题本',{confirmButtonText:'确认移出',cancelButtonText:'继续保留',type:'warning',center:true});await archiveWrongQuestion(item.questionId);ElMessage.success('已移入“已掌握/已归档”。');await load()}catch(error){if(error!=='cancel'&&error!=='close')ElMessage.error((error as ApiError).message||'归档失败。')}}
...
<style scoped>
.wrong-book-filters{display:grid;grid-template-columns:150px minmax(180px,1fr) 150px minmax(220px,1fr);gap:12px;align-items:center;margin:18px 0}.el-pagination{justify-content:flex-end;margin-top:18px}@media(max-width:980px){.wrong-book-filters{grid-template-columns:1fr 1fr}}@media(max-width:760px){.wrong-book-filters{grid-template-columns:1fr}.wrong-book-filters>*{width:100%}}
</style>
