...
<template><section class="student-page" v-loading="loading"><template v-if="result"><div class="student-page-heading"><div><h1>练习结果</h1><p>共 {{ result.totalCount }} 题，答对 {{ result.correctCount }} 题，得分 {{ result.totalScore }} 分。</p></div><el-button type="primary" @click="router.push('/student/practice/new')">再练一场</el-button></div><article v-for="item in result.questions" :key="item.question.practiceQuestionId" class="result-question" :class="item.correct?'is-correct':'is-wrong'"><div class="result-state"><el-tag :type="item.correct?'success':'danger'">{{ item.correct?'回答正确':'需要复习' }}</el-tag><span>{{ item.question.order }}. <QuestionContent :content="item.question.stem" :attachments="item.question.attachments" position="QUESTION" /></span></div><p>你的答案：{{ formatPracticeAnswer(item.question.questionType,item.studentAnswer) }}</p><p>正确答案：{{ formatPracticeAnswer(item.question.questionType,item.correctAnswer) }}</p><p>标准解析：<QuestionContent :content="item.standardAnalysis" :attachments="item.question.attachments" position="STANDARD_ANALYSIS" /></p><p class="knowledge-copy">知识点：{{ item.question.knowledgePoints.map(point=>point.path).join('；') }}</p></article></template></section></template>
