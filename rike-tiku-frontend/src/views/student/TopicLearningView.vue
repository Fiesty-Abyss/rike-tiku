...
<template>
  <section class="student-page topic-learning-page" :data-subject="environment" v-loading="loading">
    <div class="student-page-heading"><div><span class="page-kicker">TOPIC LEARNING</span><h1>主观专题学习</h1><p>每个专题包含 2 至 3 道主观题，可在页面中整理作答思路。</p></div><el-select v-model="subjectCode" class="topic-subject-select" @change="chooseSubject"><el-option v-for="option in subjectOptions" :key="option.value" :label="option.label" :value="option.value" /></el-select></div>
    <div class="topic-learning-layout">
      ...
      <main class="topic-reader">
        ...
        <article v-else>
          ...
          <div class="topic-draft"><h3>自我作答草稿</h3><p>切换专题题目会清空当前草稿。</p><el-input v-model="draft" type="textarea" :rows="8" maxlength="5000" show-word-limit placeholder="在这里整理计算、流程或分析步骤……" /></div>
    ...
    <el-dialog v-model="variantVisible" title="专题变式候选预览" width="min(680px,calc(100vw - 24px))" append-to-body>
      <el-alert title="请确认变式题内容后，再提交任课老师审核。" type="info" :closable="false" />
      ...
      <section v-if="variantTask?.candidates?.length" class="topic-variant-result"><article v-for="candidate in variantTask.candidates" :key="candidate.questionId"><div class="variant-meta"><el-tag type="warning">{{ candidate.status === 'DRAFT' ? '待提交' : '已提交审核' }}</el-tag><span>{{ questionTypeLabel(candidate.questionType) }} · 难度 {{ candidate.difficulty }} · {{ candidate.variationSummary }}</span></div><h3>候选题干</h3><QuestionContent :content="candidate.stem" position="QUESTION" /><h3>候选解析</h3><StandardAnalysis :content="candidate.standardAnalysis" /><p v-if="candidate.knowledgePoints?.length">关联知识点：{{ candidate.knowledgePoints.map(item => item.name).join('、') }}</p><StudentAiLearningPanel :topic-question-id="candidate.questionId" /><div v-if="candidate.status === 'DRAFT'" class="variant-actions"><el-button @click="discardCandidate(candidate.questionId)">丢弃候选</el-button><el-button type="primary" @click="submitCandidate(candidate.questionId)">提交任课老师审核</el-button></div><el-tag v-else type="success">已提交任课老师审核</el-tag></article></section>
