  ...
  <section class="student-page topic-learning-page" :data-subject="environment" v-loading="loading">
    ...
    <div class="topic-learning-layout">
      ...
      <main class="topic-reader">
        ...
        <article v-else>
          <header><span v-if="unit">{{ unit.subjectName }} · {{ stageLabel(activeUnitQuestion?.stage || '') }} · 第 {{ unitIndex + 1 }} / {{ unit.questions.length }} 题</span><span v-else>{{ detail.subjectName }} · 主观专题</span><h2>{{ unit?.title || detail.title }}</h2><p v-if="unit">{{ unit.introduction }}</p></header>
