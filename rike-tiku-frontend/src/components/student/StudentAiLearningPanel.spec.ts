...
describe('学生 AI 学习面板', () => {
  ...
  it('使用简短学习文案且不展示 provider、模型或 token', async () => {
  ...
  it('失败时显示简短提示并允许重试', async () => {
    ...
    const wrapper = mount(StudentAiLearningPanel, { props:{ answerFactId:19,wrong:true }, global:{ stubs, directives:{ loading:()=>undefined } } })
    await flushPromises()
    expect(wrapper.text()).toContain('AI 暂不可用，请稍后重试')
  ...
  it('创建绑定当前答题事实的会话并展示轮数，发送失败不暴露底层异常', async () => {
    ...
    expect(wrapper.text()).toContain('RIKE 理科学习助手', '当前题目答疑')
