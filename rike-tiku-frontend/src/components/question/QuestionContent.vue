...
<template><span class="question-content"><template v-for="(piece,index) in pieces" :key="index"><span v-if="piece.text">{{ piece.text }}</span><img v-else-if="piece.attachment&&sources[piece.attachment.id]" class="question-content__image" :src="sources[piece.attachment.id]" :alt="piece.attachment.fileName" /><el-tag v-else type="info" effect="plain" class="question-content__placeholder">{{ piece.attachment?.type==='IMAGE'?'图片附件暂不可用':'附件暂不支持在线显示' }}</el-tag></template></span></template>
