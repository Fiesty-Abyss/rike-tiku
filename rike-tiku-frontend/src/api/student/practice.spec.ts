...
import { createPracticeSession, fetchPracticeAvailability, fetchPracticeOptions, fetchPracticeResult, fetchPracticeSession, fetchWrongQuestion, fetchWrongQuestions, submitPracticeSession } from './practice'
...
describe('学生练习 API', () => {
  ...
  it('可用题数与类似练习使用相同受控筛选', async () => { await fetchPracticeAvailability({ subjectId:1, knowledgePointIds:[3], questionTypes:['MULTIPLE_CHOICE'], difficulty:2, referenceQuestionId:9 }); expect(adapter).toHaveBeenCalledWith(expect.objectContaining({ method:'get', url:'/student/practice-availability', params:{ subjectId:1, knowledgePointIds:[3], questionTypes:['MULTIPLE_CHOICE'], difficulty:2, referenceQuestionId:9 } })) })
  ...
  it('错题学科筛选使用稳定 subjectCode 而非数据库固定 ID', async () => { await fetchWrongQuestions('BIOLOGY'); expect(adapter).toHaveBeenCalledWith(expect.objectContaining({ url:'/student/wrong-questions', params:{ subjectCode:'BIOLOGY' } })) })
})
