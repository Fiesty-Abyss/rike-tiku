...
import { changeTeachingAssignmentStatus, createTeacher, createTeachingAssignment, fetchTeachers, resetTeacherPassword, resetTeacherPasswords, updateTeacher } from './teachers'
...
describe('教师管理 API', () => {
  ...
  it('管理员密码重置使用专用一次性响应接口', async () => { await resetTeacherPassword(5); expect(adapter).toHaveBeenCalledWith(expect.objectContaining({ method:'post', url:'/admin/teachers/5/reset-password' })) })
  it('批量恢复只提交所选教师业务 ID', async () => { await resetTeacherPasswords([5, 6]); expect(adapter).toHaveBeenCalledWith(expect.objectContaining({ method:'post', url:'/admin/teachers/reset-passwords', data: JSON.stringify({ ids: [5, 6] }) })) })
})
