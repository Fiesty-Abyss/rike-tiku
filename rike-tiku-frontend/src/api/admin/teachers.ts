...
export interface TeacherItem { id: number; employeeNumber: string; name: string; displayPosition: string | null; username: string; accountStatus: AccountStatus; profileStatus: TeacherProfileStatus }
export interface TeacherPageResponse { records: TeacherItem[]; total: number; current: number; size: number; pages: number }
export interface TeacherQuery { page: number; size: number; employeeNumber?: string; name?: string; username?: string; accountStatus?: AccountStatus; profileStatus?: TeacherProfileStatus }
export interface CreateTeacherRequest { employeeNumber: string; name: string; username: string; displayPosition?: string; initialPassword?: string; accountStatus: AccountStatus }
export interface UpdateTeacherRequest { name: string; displayPosition?: string; accountStatus: AccountStatus; profileStatus: TeacherProfileStatus }
...
export async function changeTeachingAssignmentStatus(id: number, status: TeachingAssignmentStatus) { return (await http.patch<TeachingAssignment>(`/admin/teaching-assignments/${id}/status`, { status })).data }
