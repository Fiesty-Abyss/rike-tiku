...
export interface AiModelConfig {
  id:number; provider:'DEEPSEEK'|'GLM'|'XAI'; model:string; baseUrl:string; usage:AiUsage
...
export interface SaveAiModelConfig {
  provider:'DEEPSEEK'|'GLM'|'XAI'; model:string; baseUrl:string; apiKey?:string; usage:AiUsage
  enabled:boolean; defaultConfig:boolean; timeoutMillis:number; maxTokens:number; retryCount:number
}
export interface AiConnectionResult {
  success:boolean; provider:string; model:string; latencyMillis:number; status:string
  visionSummaryPreview?:string; safeError?:string; safeErrorCode?:string; httpStatus?:number
  providerErrorCode?:string; retryAfterSeconds?:number; testedAt:string
}
