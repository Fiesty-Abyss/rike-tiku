...
- PR #27 当前机器实现已覆盖 MA-018、MA-020、管理员题目图片上传、来源权利补充、Golden30 正常导入闭环、正式菜单整理和多角色切换；本轮最终全量门禁为后端 121 个测试 0 失败、1 个 symbolic-link assumption skipped、package PASS，前端 35 个文件 133/133、type-check/build/audit 通过，Demo reset/seed/validate/smoke PASS。另已修正题干附件变更后的共用内容 hash、附件文件/数据库事务回滚清理、管理员草稿图片操作的未保存文字覆盖，以及班级 SUCCESS 审计事务一致性。Golden30 在独立测试库真实验证物理/化学/生物各 10 道从 preview、confirm 到审核发布和学生练习，原始候选文件与正式库均未被写入。
