...
## PR #37 最终认证与演示数据卫生事实（待合并 SHA 冻结）

- 203 隔离演示的教师账户为张生康（`t2026004`、`TEACHER`），指定学生编号/用户名为 `2026203001` 至 `2026203003`；199/200 及张锡鹏、吴雪莉、谢亚坤的既有教学事实不重建、不删除。
- 系统默认/管理员重置密码由配置项 `app.account.default-reset-password` 管理；本地演示为 `a1234567`，仅以 BCrypt 保存。新建、Excel 导入和重置账户不再设置首次登录强制改密，用户主动改密与管理员密码恢复仍然存在。
- 本项是认证可用性与演示数据卫生调整，不构成真实课堂教学效果或口令安全性实验结论。最终回归数字、PR 号和 main SHA 以合并后的 [FINAL_PROJECT_FACTS.md](../FINAL_PROJECT_FACTS.md) 为准。
