...
创建、编辑和状态变更均在事务内完成。审核人由认证上下文取得，不接受前端提交。题干、答案、解析来源必须齐全；存在 `COPYRIGHT_UNKNOWN` 或 `RESTRICTED` 来源时不得发布。

详情响应包含完整题干（列表仅返回摘要）、受控附件业务元数据、标准答案 JSON、标准解析、知识点、三类来源、审核历史和允许动作；附件的 `xiang_dui_lu_jing` 不向前端返回。单选/多选答案必须使用 `optionLabels` 与正确选项一致；填空答案使用 `blanks[].acceptedAnswers`；主观题只能是 `TOPIC_LEARNING` 且不可自动判分。后端专项测试共 12 项，使用随机临时 MySQL 库从 V1–V6 迁移，并另由认证 HTTP 集成测试验证未登录、学生、教师和首次未改密管理员拒绝访问，正常管理员可访问。
...
下一轮仅建议：实现 MVP30 Excel 的预检查、逐行错误、附件与知识点映射、确认入库，并使成功记录全部进入 `PENDING` 后与本页面审核流程衔接。
