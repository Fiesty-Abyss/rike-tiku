...
- PR #12 仍为 Draft、尚未合并。审查修正后，附件声明数、对象标记、唯一文件、受控来源和单学科均为阻断规则；原先物理 10/10 是旧的“图片计数只警告”结果，不再作为当前结论。严格预检查基线为物理 2/10、化学 1/10、生物 6/10 有效，任何无效文件均不会发生部分导入。
- 当前回归：后端 53/53 测试与打包通过；前端 56/56、类型检查、构建通过，`npm audit` 为 0 vulnerabilities。
