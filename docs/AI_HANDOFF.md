...
当前分支为 `feat/admin-question-review`，基线为 `main@eecae66`。已实现管理员题库草稿、审核、发布、停用、前端页面和后端/前端专项测试；不实现题库批量导入或新迁移。后端总计 43 项、前端总计 51 项测试通过。MVP30 Excel 已进行只读兼容检查，合计 30 道，原始文件和正式数据库均未改动。

后续继续时：保持 V1–V6 不变；题目详情不返回附件相对路径；创建请求不接受创建人 ID；审核人由 JWT 上下文写入 `ti_mu_shen_he_ji_lu`。`ti_mu` 不存在创建人字段，因此在不新增迁移的约束下不得声称创建人已持久化。浏览器临时库联调及 Draft PR 尚待完成。
