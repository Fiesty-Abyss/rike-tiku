...
学生主页升级为物理、化学、生物三科工作台。`/student/subjects/physics`、`/student/subjects/chemistry`、`/student/subjects/biology` 提供随机五题、条件练习和本科错题入口。创建页支持 `subjectId`、`knowledgePointId` query 预选；错题页支持 `subjectId` query 筛选，不新增或伪造学习统计。
