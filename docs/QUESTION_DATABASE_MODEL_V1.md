...
状态：当前 Flyway V1–V10、26 张业务表；PR #23 不新增迁移
...
学生练习只冻结可安全回读的 PNG/JPEG 图片附件题；附件必须位于受控相对路径、类型与 SHA-256 校验成功，题干/选项/解析对象标记须与附件关联位置一致。PDF、公式、ANSWER、缺失或不安全附件，以及对应正文含未支持对象的题目不进入普通自动判分题池。提交前不暴露 STANDARD_ANALYSIS 附件，提交后由本人结果上下文读取。
