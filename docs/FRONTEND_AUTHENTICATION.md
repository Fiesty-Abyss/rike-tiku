...
登录统一为 `/login`，历史角色路径仅兼容重定向。页面初始只显示用户名、密码、登录按钮和“忘记密码请联系管理员”，不预加载也不留出验证码区域。用户第一次点击按钮或按 Enter 时只展开 `ImageCaptcha.vue` 并聚焦输入框；验证码出现后再次提交才调用登录 API。

验证码输入框、120×40 图片和“看不清？换一张”保持一行紧凑布局；点击图片或文字都会刷新并清空旧输入。验证码错误、过期、重放或账号密码错误后自动换图，用户名与密码输入保持。challenge 只保存在组件内存中，不写入 localStorage 或控制台。

单角色登录直达，多角色进入 `/select-role`；首次改密优先进入 `/change-initial-password`。前端只允许选择 API 返回的真实角色并将 `activeRole` 放入 sessionStorage，不能影响后端授权。PR #15 的 `SliderVerification.vue` 与滑块交互已由 PR #19 当前分支删除。
