...
| MA-014 | STUDENT | 推荐练习 | 零答题时点击首个“开始巩固” | 推荐知识点应有足够可用题目 | 首次浏览器验收把无题的父级目录知识点纳入推荐 | HIGH | 统计范围改为至少关联 5 道当前可自动判分、已发布且无活动附件题目的知识点；专项断言与浏览器复验通过 | 已关闭 |
...
补充：管理员重置学生密码对应登录页“忘记密码请联系管理员”；不包含自助找回、邮件或短信。头像、个人资料与简介仍待后续；高频考点与私信已分别通过真实三元权限浏览器验收。正式库因 MA-013 当前已有 V9 两张空表，六项业务污染计数均为 0；该事实不得改写为“正式库未执行 V9”。
