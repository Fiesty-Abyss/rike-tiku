...
| MA-016 | 全部 | 公共入口 | 未登录访问根路径 | 显示公共首页、功能介绍、三科学科介绍和统一登录入口 | 根路由直接重定向 `/login`，无公共门户 | BLOCKER | V3.0 A 层；2026-08-09 路由代码与真实浏览器复现 | 未修复 |
...
补充：管理员重置学生密码对应登录页“忘记密码请联系管理员”；不包含自助找回、邮件或短信。个人中心已关闭 MA-006，不包含头像裁剪、云存储或自助修改业务档案。PR #23 浏览器抽查 10 道新变式、结果/错题/掌握度与随机变化均通过。2026-08-09 V3.0 总审计新增 MA-016 至 MA-020；最终 reset/seed/validate/smoke PASS。正式库为 V10、26 张业务表；演示题、场景账号/班级、高频考点、私信、学习及验收个人资料污染计数均为 0。
