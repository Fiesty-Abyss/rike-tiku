...
> 本文保留各轮历史验收事实，当前项目状态请查看 [README](../README.md) 和 [开发状态](DEVELOPMENT_STATUS.md)。
...
| MA-017 | ADMIN/STUDENT | 题库附件 | 导入含附件题并在详情/练习查看 | 附件可安全访问，题干或解析中真实显示 | PNG/JPEG 受控存储，SHA-256 回读校验；管理员详情及学生题面/结果/错题统一渲染。未提交会话拒绝 STANDARD_ANALYSIS；PDF 仍不进入普通练习 | BLOCKER | 后端附件/权限/导入/题池专项 27 个，26 PASS、1 个符号链接 assumption skipped；全量 112 个测试 0 失败、1 skipped、package PASS；前端附件专项 4/4、全量 127/127、type-check/build/audit；Demo `reset → seed → validate → smoke` PASS，PHYSICS-S1 两条附件实际文件/hash 回读通过。用户已完成最终 acceptance 环境人工浏览器复验 | 已关闭 |
| MA-018 | ADMIN | 高风险操作日志 | 执行账号、题库、审核等管理员写操作 | 关键高风险操作可追溯 | PR #27 已加入 V11 日志表、真实 Service 记录和 ADMIN 查询 API/页面，覆盖当前管理员高风险写操作 | BLOCKER | 日志专项已验证真实操作者、成功/失败、脱敏摘要、ADMIN 查询权限；用户已完成最终管理员浏览器复验；正式库未写入 | 已关闭 |

| MA-019 | ADMIN | 题库批量导入 | 用 30 道合法样例完成预检查、确认、审核、发布、查询和附件显示 | 全链路可重复验收 | PR #27 新增独立 Golden30 测试，复用三科清洗候选素材，在隔离库真实经过 preview、confirm、来源权利补充、审核、发布、查询、附件和学生练习；物理/化学/生物各 10 道 | BLOCKER | Golden30 测试通过；30 道均发布，29 道固定答案题进入自动练习，1 道主观题保留专题学习；用户已完成最终管理员导入与题库浏览器复验；原始 Excel SHA-256 未改变，正式库未写入 | 已关闭 |
| MA-020 | STUDENT | 练习提交 API | 对提交接口发送空请求体 | 返回明确 4xx 校验错误 | 空 body 与不可解析 body 均返回 `400 INVALID_REQUEST`；正常提交、判分和重复提交回归不变 | MEDIUM | `RenZhengJiChengTest.emptyOrUnreadablePracticeSubmitBodyReturnsBadRequest` 通过；用户已完成最终练习提交与结果浏览器复验；正式库未写入 | 已关闭 |
| MA-021 | PUBLIC/STUDENT/TEACHER | Portal 与学科环境 | 查看 Portal、三科学生页与教师任课范围 | 形成成熟、原创、清楚分科且克制的学术产品视觉 | Portal 已改为 Hero、物理、化学、生物、学习闭环、入口六场景；四张原创 Aqua WebP 主视觉进入静态构建，学生/教师继续由真实 `subjectCode` 驱动整页环境；管理员保持非学科中性 | UX | Round 4 机器浏览器证据与用户最终 Portal、三科学科和工作台人工复验均通过 | 已关闭 |
| MA-022 | STUDENT | 学习掌握 | 查看“已练习知识点 / 总知识点” | 使用自然内联比例，例如 `0 / 38` | 该语境已从上下堆叠分数改为内联比例，计算、API 和 MetricFraction 其他用途不变 | UX | 前端专项断言 `0 / 38`；机器浏览器与用户最终学生端人工复验均确认 `0 / 38` 与 aria-label | 已关闭 |
| MA-023 | STUDENT | 结果/错题与 Demo360 解析 | 查看单选、多选答案和标准解析 | 显示冻结的完整选项，并逐项说明全部活动选项 | `AnswerDisplay` 统一以练习/错题冻结快照显示 `label + 内容`，缺失内容安全回退 label；Demo 246 道选择题的确定性解析均含结论、4 项判断、题目依据和易错点 | HIGH | 全量门禁确认 246/246 非空、每个活动选项均出现、246 份解析唯一，三科各固定抽样 10 道；用户已完成三科结果与逐项解析人工抽查 | 已关闭 |
| MA-024 | STUDENT | Topic18 | 展开标准解析 | 使用可读的多段标题、步骤、结论和易错点 | Topic18 全部 18 道 seed 解析改为学科化多段结构；`StandardAnalysis` 按换行安全渲染并复用 ScientificText / KaTeX，不使用 `v-html` 或 Markdown HTML | HIGH | 后端全量结构门禁 18/18；前端组件测试覆盖多段、公式和普通旧文本；机器浏览器与用户三科 Topic 人工复验均通过 | 已关闭 |
| MA-025 | STUDENT | 生物填空判分 | canonical 为 `1/2` 时提交 `50%` | 在该空显式声明等价时判为正确 | 复用现有 accepted answers，为 BV-06 明确列出 `1/2`、`0.5`、`50%`、`50％`；未增加求值器、迁移或 AI 判分 | HIGH | 后端覆盖正确/错误边界、普通文本、多空、数量、单选/多选回归；机器浏览器与用户生物填空人工复验均确认 `50%` 判定正确 | 已关闭 |
| MA-026 | PUBLIC/AUTH/STUDENT/TEACHER/ADMIN | 全站视觉系统与动态叙事 | 查看 Portal 六场景、登录/角色入口、三科学生与教师环境、管理员工作台，并体验桌面滚动与 390px 降级 | 第一眼具有水、空气、透明光学和自然科技共生的 Frutiger Aero；三科科学语义清楚；至少一个连续 scroll-driven 与合理 pinned scene；工作区仍清楚高效 | Round 3 视觉验收失败后，Round 4 以 `RIKE Aqua Future` 重建设计 token、材质、排版、共享 shell、Portal/Auth 与三角色工作台；四张原创 Aqua WebP、Physics pin+scrub、学科环境转场、pointer 光学反馈、移动端与 reduced-motion 降级均已进入当前分支，MA-022 至 MA-025 业务能力未回退 | UX | Round 4 前端门禁为 50/50 文件、174/174 测试，type-check/build PASS、audit 0；机器截图与 motion 说明位于 `docs/evidence/pr27-ui-round4/`；用户已完成最终 Portal/Auth/Student/Teacher/Admin 人工复验 | 已关闭 |
...
PR #27 第三轮最终集成人工验收前机器准备（2026-08-11，历史执行结果）：后端全量 133 个测试，0 failure、0 error、1 个 symbolic-link assumption skipped，package PASS；前端当时为 49 个文件、170/170，type-check、build、audit 0。Demo `acceptance-prepare → validate → smoke` PASS，固定 Demo360 为物理/化学/生物各 120 道、55 个叶子知识点，另有 Topic18，总题量 378；PHYSICS-S1 两条图片附件文件与 SHA-256 回读通过。第三轮 production-like preview 曾复验 Portal 1280/390、三科章节、学生/教师三科学科环境、管理员中性 Dashboard、内联掌握度、冻结完整答案、逐项解析、三科 Topic 和生物 `50%` 正确判分，最终完整复跑 console/page error/HTTP 4xx+ 均为 0。截图见 `docs/evidence/pr27-ui-round3/`。这些机器结果没有改变随后发生的第三轮视觉人工验收失败。

PR #27 Round 4 视觉机器修正（2026-08-11，当前前端门禁）：`npm test -- --run` 为 50/50 文件、174/174 测试 PASS，`npm run type-check` PASS，`npm run build` PASS，`npm audit --omit=dev` 为 0 vulnerabilities；Vite 如实报告 main chunk 797.43 kB、gzip 254.25 kB，超过 500 kB 的 warning 未隐藏。Round 4 未修改后端 Java、API contract、Flyway 或数据库，因此没有机械重跑 `mvn clean test/package`；上一段 133 个测试、1 个 symbolic-link assumption skipped 和 package PASS 只作为最近一次后端执行事实保留。MA-017 继续为 `IMPLEMENTED_AWAITING_FINAL_MANUAL_ACCEPTANCE`，MA-021 至 MA-025 保留 `FIXED_AWAITING_USER_RETEST`，MA-026 为 `FIXED_AWAITING_USER_RETEST`；非 AI 整体仍不得写成 `DONE_VERIFIED`，PR #27 继续 Draft，AI 未开始。
...
第四轮视觉人工反馈与机器修正（2026-08-11）：用户正式判定 Round 3 视觉人工验收失败，指出普通极简 SaaS、首屏无效留白、学科环境不足、Frutiger Aero 材质缺失以及 motion 只停留在 fade/reveal。该反馈新增 MA-026。Round 4 已以 `RIKE Aqua Future` 替换旧 Split Studio：Portal 形成 Hero、Physics、Chemistry、Biology、Learning Loop、Entrance 六场景，Physics 使用 continuous scrub 与 pinned scene，导航/材质/学科环境连续变化；Auth、Student、Teacher、Admin 共享新 token 与角色边界，移动端和 reduced motion 有完整降级。上述为机器修正完成，不是用户复验 PASS；MA-026 当前为 `FIXED_AWAITING_USER_RETEST`。

## PR #27 最终封板记录（2026-08-11）

用户已在最终 acceptance 环境完成真实图形 CAPTCHA、Portal 1440/1280/390、三科学科页面、Student、Teacher、Admin、Multi-role、练习结果、Topic18 与生物 `50%` 判分复验，并确认没有继续阻塞非 AI 阶段的问题。该结果与机器测试、机器浏览器证据分开记录：机器测试证明结构和回归门禁；机器浏览器证明页面与控制台取证；本段记录用户本人最终浏览器复验。

因此 MA-017、MA-018、MA-019、MA-020、MA-021、MA-022、MA-023、MA-024、MA-025、MA-026 全部正式状态为 `已关闭`。PR #27 是最后一个普通非 AI 工程 PR，非 AI A 层正式标记为 `DONE_VERIFIED`。后续 AI 阶段必须从合并后的最新 `main` 进入，不回改历史审计。
